...
from app import db
from app.valves.forms import ValveForm, EditValveForm
from app.models import Valve
from app.valves import bp
...
def valves():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('valves.valves'))
    page = request.args.get('page', 1, type=int)
    valves = Valve.query.order_by(Valve.id).paginate(
        page, current_app.config['VALVES_PER_PAGE'], False)
    next_url = url_for('valves.valves', page=valves.next_num) \
        if valves.has_next else None
    prev_url = url_for('valves.valves', page=valves.prev_num) \
        if valves.has_prev else None
    return render_template('valves/valves.html', title='Sprinkler Valves', form=form,
...
def edit_valve(valve):
    ...
    return render_template('valves/edit_valve.html', title='Edit Valve',
                           form=form)