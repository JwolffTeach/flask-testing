...
@login_required
def checkboxes():
    form_days = Schedule_Days_Form()
    form_valves = Schedule_Valves_Form()
    Valves = ZoneSchedule.query.all()
    if form_valves.validate_on_submit():
        formData = request.values
        print(request.values)
        flash(request.values)
        return redirect(url_for('schedule.checkboxes'))
    return render_template('schedule/checkboxes.html', title='CheckBoxes', form_days=form_days, Valves=Valves, form_valves=form_valves)
...
def edit_schedule(schedule):
    ...
    #valves = ZoneSchedule.query.join(Valve, ZoneSchedule.zone == Valve.valve).add_columns(Valve.valve, Valve.description).filter_by(valve=schedule).first()
